...
XXX: Flush out the audit data more.  Entries should be have a well defined
structure that make them easy to process, filter, aggregate, etc.  However
it would be helpful to include at least one free form field in the audit data.
For example, once a token has been disposed of, an operator will want to
change its state to `deleted` to prevent any further use, but may also want
to annotate the deletion event with some other information surrounding the
deletion event.
...
The combination of `model` + `serial` fields should be unique.  Likewise
the `GUID` field should be unique across all `assigned` entries.

The following table shows the which fields are present based on the `state`
field of token:
...
| current.attestation
| Not Present
| Conditional footnoteref:[attest,If the token supports attestation, this is
required, otherwise it is omitted]

| current.attestation.9a | Not Present | Optional foot  noteref:[attest]
...
| history
| Required footnoteref:[empty,May be empty]
| Required footnoteref:[empty]
| footnoteref:Required[empty]
...
If the request appears to be a duplicate CreateToken request (all of the fields
in the request match an existing entry), the `recovery_token` value of the
existing entry is returned **as long as the signature check succeeds**.  This
allows a CN to retry a CreateToken request in the event the response is lost.
The token data in KBMAPI itself is not changed in this situation.
