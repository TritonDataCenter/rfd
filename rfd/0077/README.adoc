...
Additionally, an operator may optionally decide to limit the tokens allowed
...
deleting the conflicting entry is unlikely to be desirable.  Perhaps checks
can be run during CN boot that would alert of any mismatch, possibly requiring
resolution before the boot can proceed.
