...
a Manta installation using hagfish.
...
match an existing entry.  In either case, a 409 Conflict + NotAuthorized error
is returned.  In such an instance, an operator must manually verify if the
information in the token bucket is out of data and manually delete it.  Since
we have seen duplicate CN UUIDs appear in the past, we don't want to overwrite
the existing data in the event the existing entry is legitimately referring to
another CN with a duplicated UUID.
