...
(described in detail below) provide the means for adding new tokens,
removing tokens, recovering tokens (i.e. replacing a token), as well as
providing the PIN of a token to an authenticated entity.
...
devices (essentially authenticating the serial number of the device).

#### Token object

The data managed by KBMAPI must be persistent (for hopefully obvious reasons).
Currently, we plan to use Moray buckets to store this information.  A possible
form of the token object (as stored in Moray) is:
...
API call to obtain it's pin, should we still go ahead and store its data in
KBMAPI?

To accomplish the goals of being able to add new tokens, remove tokens,
recover tokens, and obtain the pin of a token, the following REST endpoints
are proposed:
...
least the 9E key (probably others).
...
The endpoints necessary for recovery are less well defined currently.  In a
recovery situation, the original token for a CN is unavailable due to damage,
loss, etc. and must be replaced with a new token.  Since the original token
is unavailable to authenticate the CN, the recovery token generated during the
original CreateToken call (and securly saved on the CN) is used to authenticate
the recovery process.

An open question is what should this API call look like?  PUT /pivtokens/:guid
is a possibility.  We need the guid of the old piv (this could be obtained via
a ListToken call filtering on the CN's uuid, or could be stored locally on the
CN), the recovery token, and then the information for the new token (the same
information as during a CreateToken request).  We may want the format of the
request to look slightly different, such as:
...
that way what pieces are about the old token and which are about the new
token are clear.  It's not strictly necessary (we could just use the exact
same format as CreateToken, but include the `recovery_token` field), but seems
less likely to lead to confusion.  We create a new recovery token to accompany
the new token, and remove the old token.
