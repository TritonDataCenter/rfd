...
the head node.  Once complete, the operator proceeds to re-run the setup
...
tracked as {OS-7633}[OS-7633].  In short, a new property `CN Encrypted` will
...
This assumes that DAPI is able to use the `CN Encrypted` property from sysinfo,
as described in {OS-7633}[OS-7633].
