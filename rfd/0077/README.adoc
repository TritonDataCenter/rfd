...
3. Notify any local users of the PIV token of the change.  Interested local
   users will issue a 'register' command along with the fd of a door server
   in the interested program.  kbmd will use this door server to send the
   notification (most of this will likely be abstracted behind a library
   interface to minimize the amount of work required by the interested party).
...
. Execute `zpool create` prepending the arguments necessary to enable
  encryption on the pool, obtain the key from stdin, and set the `rfd77:config`
  property to the base64 encoded value of the ebox.  kbmd will write the
  random encryption key generated in the earlier step to the stdin of the
  `zpool create` command so that it's not readily observable from the running
  system.

The ultimate `zpool create` command that executes will look similar to:
...
The `kbmadm create-zpool` command will block until the zpool has been created
or an error occurs.  It is important to note that the `kbmadm` command sends
the create request to `kbmd` and `kbmd` does the actual work.
