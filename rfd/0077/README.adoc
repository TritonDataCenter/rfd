...
the `/.zonesec` directory. This will be a lofs-mounted directory in the same
style as `/.zonecontrol`.

The existing `/.zonecontrol` directory cannot be used, because the filesystem
permissions on the top-level directory are currently used to control access to
the metadata socket today (which itself has no authentication). The `/.zonesec`
directory will be world-readable and world-traversable, and will use privileges
rather than filesystem permission to control access.

The socket file itself within `/.zonesec` will be named `soft-token-agent` (i.e.
its full path will be `/.zonesec/soft-token-agent`). The socket file will be
world-writable and world-readable.

Upon a connection being made by a client process, the soft token daemon will
examine the `cred_t` of the connecting process. A new system-wide privilege
bit, `PRIV_SMARTOS_SOFT_TOKEN` will be added, and any connecting process in
possession of this privilege bit will be allowed to use the soft token.
